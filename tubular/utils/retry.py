...
def retry(attempts=MAX_ATTEMPTS, delay_seconds=DELAY_SECONDS, max_time_seconds=MAX_TIME_SECONDS):
    """
    Decorator wraps a function that will attempt to "retry" the function if an exception is raised during exeuction.
...
class LifecycleManager(object):
    ...
    def max_attempts_reached(self):
        """
        Returns:
            bool: True if the retry has reached it's maximum number of attempts
                  False if the current attempt is less than the number of attempts
        """
        if self._current_attempt_number <= self.max_attempts:
            return False
        else:
            return True
    ...
    def max_time_reached(self):
        ...
        """
        if self._max_datetime and self._max_datetime - datetime.utcnow() < timedelta(0, 0, 0):
            return True
        else:
            return False
    ...
    def get_delay_time(self):
        ...
        """
        return round(self.delay_seconds)
    ...
    def execute(self, fn, *args, **kwargs):
        ...
        """
        while not self.max_attempts_reached() and not self.max_time_reached():
            try:
