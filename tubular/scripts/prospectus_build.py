...
"""
Command-line script to build a prospectus application.
"""
...
SCRIPT_SHORTNAME = 'Build prospectus'
...
@click.option(
    '--app-name',
    help='Name of the prospectus app.',
)
@click.option(
    '--version-file',
    help='File to which to write app version info to.',
)
def prospectus_build(env_config_file, app_name, version_file):
    """
    Builds a prospectus application.

    Uses the provided environment-specific configuration file to pass
    environment variables to the prospectus build.

    Args:
        env_config_file (str): Path to a YAML file containing environment configuration variables.
        app_name (str): Name of the prospectus app.
    ...
    LOG(
        'Prospectus app {} built successfully with config file {}.'.format(
            app_name,
            env_config_file,
        )
    )
...
if __name__ == "__main__":
    prospectus_build()  # pylint: disable=no-value-for-parameter
